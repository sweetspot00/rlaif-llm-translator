...
\title{Context-Aware and Physics-Align LLM-Social Force Model (CAPL-SFM) Crowd Simulator}
...
\begin{abstract}
    We present LLM-SFM Translator, a context-aware and physics-aligned framework for trajectory generation and crowd simulation in unseen scenarios. Given scene images and natural-language prompts, the proposed method translates high-level contextual descriptions into physically grounded Social Force Model parameters and agent trajectories. To improve behavioral fidelity and physical consistency, we incorporate reinforcement learning from AI feedback (RL-AIF) to distill reasoning and decision patterns from large language models into a stable translator. We further introduce a novel evaluation framework with a benchmark dataset that combines statistical metrics, agent-level behavioral consistency, LLM-based judgment, and human evaluation. Experiments on standard and unseen environments demonstrate improved generalization, interpretability, and physical plausibility over existing learning-based and controllable simulation baselines.
\end{abstract}
...
\subsection{LLM Physics Parameters Translator}
...
To get a dataset that could demonstrate crowd in public area, we scripted around 100 world famous public places maps including tourists-heavy area, stadiums, university campus and public transportation hub. We then implemented an image to obstacle data processing pipeline using the colormap provided by Google Map official API as well as segmentation model and Polygon approximity algorithm. 
The script real world map size is fixed with a width around 301.7 m, height around 282.8 m.
% figure 1: real world map to obstacle map
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/real_world_to_obstacle.jpg} % path relative to paper/
  \caption{Real world map to obstacle map.}
  \label{fig:real-world-obstacle}
\end{figure}

To generate the contextual description of crowd scenarios, we categorize the crowd into different types based on Berlonghi (TODO)'s crowd 
psychological theory, including casual gathering, spectator crowd, expressive crowd, and aggressive crowd. In total we have 11 categories of different crowd.
We then designed a prompt template to let GPT-5.1 to generate high level and diverse natural language descriptions for each type of crowd scenario. In total we gathered
around 50k context descriptions. 

To further quantify the results of the crowd simulation, we also let GPT-5.1 to generate possible exits and event center, together with 
their pixel coordinates in the map. Table 1 shows all the information we designed to let gpt-5.1 generate.
...
\begin{table}
    ...
    \begin{tabular}{lll}
        ...
        \hline
        \texttt{scenario}     & Contextual descriptions generated by GPT-5.1 & An explosive sound echoes from the right wing people surge away into the left and lower open zones. \\
        \texttt{category}     & The crowd category that this scenario belongs to & Violent  \\
        \texttt{crowd\_size}  & Agents numbers inside this map   & 100-500 \\
        \texttt{event\_center} & Pixel coordinates of the synthetic event center of this scenario; Could also be a distribution & $[560, 340]$ \\
        \texttt{goal\_location} & Pixel coordinates of the synthetic goal locations based on the scenario; Could also be a distribution & gaussian \\
        \hline
    \end{tabular}
    \caption{Attributes inside the synthetic dataset.}
    \label{tab:plain}
\end{table}
